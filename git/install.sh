...
gc sendemail.aliasesfile ~/.conf/git/mailalias
...
gc alias.clean-branches '!f() { git for-each-ref --merged=origin/master refs/heads|cut -d "	" -f 2|grep -v refs/heads/master|cut -d / -f 3- | xargs git branch -d; }; f'
